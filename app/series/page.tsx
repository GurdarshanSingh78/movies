import Link from 'next/link';
import GenreFilter from '../components/GenreFilter';

export default async function SeriesPage({ searchParams }: { searchParams: Promise<{ genre?: string }> }) {
  const resolvedSearchParams = await searchParams;
  const selectedGenreId = resolvedSearchParams?.genre;
  const TMDB_KEY = process.env.TMDB_API_KEY;

  // 1. Fetch TV-specific genres LIVE from TMDB
  const genreRes = await fetch(`https://api.themoviedb.org/3/genre/tv/list?api_key=${TMDB_KEY}`);
  const genreData = await genreRes.json();
  const genres = (genreData.genres || []).map((g: any) => ({
    id: g.id.toString(),
    genre_name: g.name
  }));

  // 2. Target the trending TV endpoint or look up the top-rated variants by genre tag
  let fetchUrl = `https://api.themoviedb.org/3/trending/tv/day?api_key=${TMDB_KEY}`;
  if (selectedGenreId) {
    fetchUrl = `https://api.themoviedb.org/3/discover/tv?api_key=${TMDB_KEY}&with_genres=${selectedGenreId}&sort_by=popularity.desc&vote_count.gte=100`;
  }

  const titlesRes = await fetch(fetchUrl);
  const titlesData = await titlesRes.json();
  const liveTitles = titlesData.results || [];

  return (
    <div className="max-w-7xl mx-auto py-8">
      <div className="flex flex-col gap-2 mb-6">
        <h1 className="text-3xl font-extrabold tracking-tight md:text-4xl">TV Series</h1>
        <p className="text-zinc-400 text-sm">Browse international shows and seasonal series.</p>
      </div>

      <GenreFilter genres={genres} />

      {liveTitles.length === 0 ? (
        <div className="text-center py-12 text-zinc-500">
          No television shows found for this genre selection.
        </div>
      ) : (
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6">
          {liveTitles.map((show: any) => (
            <Link 
              href={`/title/${show.id}?type=tv`}
              key={show.id} 
              className="group relative flex flex-col bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden hover:scale-105 hover:border-zinc-700 transition-all duration-300 shadow-lg cursor-pointer"
            >
              <div className="relative aspect-[2/3] w-full bg-zinc-800">
                {show.poster_path ? (
                  <img 
                    src={`https://image.tmdb.org/t/p/w500${show.poster_path}`} 
                    alt={show.name} 
                    className="object-cover w-full h-full group-hover:opacity-80 transition-opacity" 
                    loading="lazy" 
                  />
                ) : (
                  <div className="flex items-center justify-center h-full text-xs text-zinc-500">
                    No Poster
                  </div>
                )}
              </div>
              <div className="p-3 flex flex-col justify-between flex-grow bg-gradient-to-t from-zinc-950 to-zinc-900">
                <h2 className="font-semibold text-sm line-clamp-1 group-hover:text-amber-400 transition-colors">
                  {show.name}
                </h2>
                <div className="flex items-center justify-between mt-1.5 text-[11px] text-zinc-400">
                  <span>
                    {show.first_air_date ? new Date(show.first_air_date).getFullYear() : 'N/A'}
                  </span>
                </div>
              </div>
            </Link>
          ))}
        </div>
      )}
    </div>
  );
}
